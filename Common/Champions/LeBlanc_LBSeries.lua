...
local nmy = {}
local KillText = {}
local Position = {"Out of Range", "in double W range", "in W range"}
local colorText
...
local function WvL(o)
	print("LONG")
	local Pos = GetOrigin(o) or o
	if W1RDY == 1 then CastSkillShot(_W,Pos.x,Pos.y,Pos.z) end
	print("Time "..GetTickCount())
	DelayAction(function()
		print("Delay start "..GetTickCount())
		if W3RDY == 1 then 
			print("OK "..GetTickCount())
			CastSkillShot(_R,Pos.x,Pos.y,Pos.z)
		end
	end, 500)
end
------------------------------------------
...
------------------------------------------
--Combo Stuff im sorry for cp but that droven my crazy
------------------------------------------
...
local function AnalyzeSituation(enemy)
	...
	if Dist > 1475 and Dist < 2050 and W1RDY == 1 and Mana(0,1,0) == 1 and RRDY == 1 then
		B = Position[2]
	elseif Dist > 2050 and Position[1] or Dist < 1475 and Dist > 725 and ((W1RDY == 1 and Mana(0,1,0) == 1) or W3RDY == 1) then
		B = Position[3]
	elseif Dist < 725 then
		B = Position[4] 
	else
		B = Position[1]
	end
...
local function GetDamage(Skill, enemy)
	if enemy then
		...
		if RRDY == 1 and (Skill == "doR") and Q2RDY == 1 then
			TotalMagicDamage = TotalMagicDamage + xR
		end
		if RRDY == 1 and (Skill == "doR") and W3RDY == 1 then
			TotalMagicDamage = TotalMagicDamage + xRW
		end
		if RRDY == 1 and (Skill == "doR") and E2RDY == 1 then
			...
			if CCCheck(enemy) then
				...
				if W1RDY == 1 then
					if LeBlanc.Keys.Priority:Value() == (1 or 2) then
						...
						--print("QRE short")
					elseif LeBlanc.Keys.Priority:Value() == (3 or 4) then --Cast W before all
						if LeBlanc.Keys.Priority:Value() == 3 then --Cast E then
							bestcombo = {"doW", "doR" ,"doE", "doQ"}
							--print("REQ short")
						elseif LeBlanc.Keys.Priority:Value() == 4 then --Cast Q then
							bestcombo = {"doW", "doR" , "doQ", "doE"}
							--print("RQE short")
						elseif LeBlanc.Keys.Priority:Value() == (5 or 6) then --Cast Q then
...
local function CastSkill(Skill, enemy)
	if Skill == "doQ" and Q1RDY == 1 then
		if GetDistanceSqr(GetOrigin(enemy)) > 562500 or Q1RDY == 0 then
			...
			Q(enemy)
		return true
	elseif Skill == "doW" and W1RDY == 1 then
		local resultB = AnalyzeSituation(enemy)
		...
		elseif resultB == Position[3] then
			...
			if WPred.HitChance == 1 then
				if mapID == SUMMONERS_RIFT then
					if not MapPosition:inWall(Point(WPred.PredPos.x, WPred.PredPos.y, WPred.PredPos.z)) then
						--print("Doing W")
						W(enemy)
						return true
					else
	...
	elseif Skill == "doE" and E1RDY == 1 then
	...
	elseif Skill == "doR" and RRDY == 1 then
	...
	elseif Skill == "IGNITE" and IRDY == 1 then
...
OnTick(function(myHero)
	...
	if LeBlanc.Keys.Combo:Value() then
		Combo()
	elseif LeBlanc.Keys.Harass:Value() and target then 
