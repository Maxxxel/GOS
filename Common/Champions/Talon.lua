...
function Killsteal()
	for i,enemy in pairs(GOS:GetEnemyHeroes()) do
		if enemy and GOS:ValidTarget(enemy,25000) then
-------------------------------------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------------------------------
			...
			if Config.Note:Value() then
				DrawCircle(targetPos.x,targetPos.y,targetPos.z,50,0,0,0xffff0000)
...
-------------------------------------------------------------------------------------------------------------------------
			if Config.KS:Value() and ((GotBuff(enemy,"brandablaze")==0) or (GotBuff(enemy,"brandablaze")~=0 and enemyhp>PDMG)) then
				local QPred2 = GetPredictionForPlayer(GetOrigin(myHero),enemy,GetMoveSpeed(enemy),(math.floor(math.random()*400)+1600),250,950,60,true,true)
